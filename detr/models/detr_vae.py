...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, action_dim, num_queries, camera_names):
        ...
        Parameters:
            ...
            state_dim: robot state dimension of the environment (input qpos dimension)
            action_dim: robot action dimension (output action dimension)
...
class CNNMLP(nn.Module):
    def __init__(self, backbones, state_dim, action_dim, camera_names):
        ...
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
            transformer: torch module of the transformer architecture. See transformer.py
            state_dim: robot state dimension of the environment (input)
            action_dim: robot action dimension (output)
            num_queries: number of object queries, ie detection slot. This is the maximal number of objects
                         DETR can detect in a single image. For COCO, we recommend 100 queries.
            aux_loss: True if auxiliary decoding losses (loss at each decoder layer) are to be used.
        """
...
def build(args):
    ...
    action_dim = args.action_dim if hasattr(args, 'action_dim') else state_dim
